...
最终修复波的定向复审发现：`fetch.crawl` 吞掉单页异常后退出码为 0、且失败页从 manifest 消失，CI 会静默发布残缺索引且不触发整脚本重试。用户裁定方向：

1. **单页级重试**（`wiki.py`）：`HtmlClient.fetch_page` 对瞬态失败（非 404 的 HTTPError、网络错误）在**单页层面**重试，最多 `FETCH_RETRIES` 次、间隔 `FETCH_RETRY_DELAY` 秒（模块级**静态常量**，便于日后调整；构造函数可覆写，供测试注入 0 延迟）。404 不重试；全部失败才抛出。整脚本重试（CI 的 `nick-fields/retry`）只用于整轮失败（Anubis/网络整体不可用），不再承担单页失败。
2. **失败页保留旧记录**（`fetch.py`）：单页重试后仍失败的页，若上一版 manifest 有记录，则原样保留（raw 文件本就还在），使 `build` 不丢页；`crawl` 仍以退出码 0 结束并打印失败清单（单页失败不应触发整脚本重试）。
3. 规格 §11 补充：单页在客户端层重试 N 次（模块常量），仍失败则保留原记录、计入失败清单。