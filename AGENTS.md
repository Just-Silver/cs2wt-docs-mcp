...
- **主键是 `title`**（HTML 拿不到 `pageid`）。raw 为 `data/raw/<slug>.html`，`slug = urllib.parse.quote(title, safe="")`；`data/`、`cookies.txt`、`*.sqlite`、`.superpowers/`、`.codegraph/` 均被 gitignore，**不要提交**。
...
- 旧 `data/docs.sqlite`（`pageid` schema）**不兼容**：迁移时删库、删 `data/raw/*.wiki`、**保留** `manifest.json`，再 `cs2wt fetch` + `cs2wt build`（详见 README「从旧版本迁移」）。
