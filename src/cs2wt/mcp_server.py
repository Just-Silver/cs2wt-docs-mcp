...
def build_server(manager: IndexManager) -> MCPServer:
    ...
    def search_docs(query: str, limit: int = 10) -> str:
        ...
        何时不使用：已知道确切页面标题（或数字 rowid）时，直接调用 get_page 更高效；
        与 CS2 Workshop Tools 文档无关的问题不要使用。
        返回：JSON，含 count 与 results（每项含 title、url、snippet、score；
    ...
    def get_page(id_or_title: str, section: str | None = None) -> str:
        ...
        何时使用：已知页面标题（或数字 rowid），需要阅读其内容；或只想读取某章节以控制返回体积。
        何时不使用：只知道模糊关键词时，先用 search_docs 定位。
        参数：id_or_title 为纯数字时按数字 rowid 查询，否则按页面标题查询；
        section 为可选的章节标题（不区分大小写，支持部分匹配）。
        返回：JSON，含 found、title、url、revid、timestamp、content；
    ...
    def list_pages() -> str:
        ...
        返回：JSON，含 count 与 pages（每项含 title）。索引初始化中或刷新失败时返回
